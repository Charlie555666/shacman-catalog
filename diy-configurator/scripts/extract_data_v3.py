"""Extract ALL vehicles with smart dual-column price detection - v3"""
import pandas as pd
import json
import re
import os
...
# ── 1. All Vehicles from Knowledge Base ──
...
# Smart price extraction: check both price_raw and options_raw columns
vehicles = []
...
print(f"Countries found: {len(country_vehicles)}")
...
print(f"Engine swaps: {len(engine_swaps)} rules, {len(engine_lookup)} engines with alternatives")
...
# ── 4. Paint Colors ──
paints = {
    "X6000": {"standard": ["冰川白", "薄雾紫", "土灰色", "星际银", "RAL9005黑色", "RAL2004橙色", "霓光红", "清水蓝"]},
    "X5000_X3000": {"standard": ["冰川白", "闪电橙", "炽烈金", "土灰色", "星际银", "极光白", "霓光红", "公爵黄", "青山绿", "克莱因蓝"], "optional": [{"name": "糖明黄", "price": 1500}, {"name": "阿曼蓝", "price": 1500}, {"name": "RAL3020红", "price": 1000}, {"name": "夜空蓝", "price": 1000}, {"name": "RAL1002沙黄", "price": 1000}, {"name": "活力绿", "price": 1500}, {"name": "清水蓝", "price": 1500}, {"name": "RAL2004橙色", "price": 1000}, {"name": "RAL9005黑色", "price": 1000}, {"name": "薄雾紫", "price": 1500}, {"name": "银灰色", "price": 1500}]},
    "F3000_H3000_tractor": {"standard": ["闪电橙", "炽烈金", "极光白", "霓光红", "公爵黄", "克莱因蓝", "青山绿"], "optional": [{"name": "糖明黄", "price": 1500}, {"name": "阿曼蓝", "price": 1500}, {"name": "RAL3020红", "price": 1000}, {"name": "夜空蓝", "price": 1000}, {"name": "RAL1002沙黄", "price": 1000}, {"name": "清水蓝", "price": 1500}, {"name": "土灰色", "price": 1500}, {"name": "活力绿", "price": 1500}, {"name": "星际银", "price": 1500}]},
    "F3000_H3000_dump": {"standard": ["极光白", "霓光红", "公爵黄", "克莱因蓝", "青山绿"], "priced_standard": [{"name": "闪电橙", "price": 800}, {"name": "炽烈金", "price": 800}]},
    "custom_color_price": 3000
}
with open(os.path.join(OUT, "paints.json"), "w", encoding="utf-8") as f:
...
print(f"\n{'='*50}")
print(f"SHACMAN DIY Configurator Data Summary v3")
print(f"{'='*50}")
print(f"Countries: {len(country_vehicles)}")
print(f"Vehicles:  {len(vehicles)} ({sum(1 for v in vehicles if v['has_price'])} priced)")
